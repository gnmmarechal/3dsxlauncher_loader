...
Result patchPayload(u32 payloadsize){
	ProgramID titleID;
	aptOpenSession();
	APT_GetProgramID((u64*)&titleID);
	aptCloseSession();
	u32 uniqueID = titleID.uniqueID>>8;
	char payloadPath[10];
	sprintf(payloadPath,"pl/%lX",uniqueID);
	...
	if (offset >= payloadsize) return -1;
	printf("Patching payload offset 0x%lX...\n",offset-9);
	memcpy(&filebuffer[offset-9],payloadPath,9);
	return 0;
}

void deHookExit(){
	FILE* f = fopen("sdmc:/pl/state","r");
	if (f != NULL){
		fclose(f);
		unlink("sdmc:/pl/state");
	}
}
...
int main(int argc, char **argv)
{
	...
	if(ret!=0 && ret!=0xd8a0a046)printf("An error occured, please report this to here if it persists: http://rinnegatamante.it/board/showthread.php?tid=90\n");
